...
def get_data(last_date, period_lag, period_term):
    ...
    train_data.iloc[:,:-1] = train_data.iloc[:,:-1].shift(period_lag, freq=period_term)
    
    # Удаляем пустые строки
    train_data = train_data[train_data.notnull().all(axis=1)]
    
    # Берем последние N строк до определнной даты
    train_data = train_data[:last_date][-300_000:]
...
for single_date in daterange(start_date, end_date):
    ...
    X, y = get_data(last_date, 15, 'min')
    LGB_result = LGB_kfold(X, y, param, n_splits)
    with open(f'models\kat_15m\model_{last_date}.pkl', 'wb') as f:
        pickle.dump(LGB_result, f)

    X, y = get_data(last_date, 3, 'H')
    LGB_result = LGB_kfold(X, y, param, n_splits)
    with open(f'models\kat_3h\model_{last_date}.pkl', 'wb') as f:
        pickle.dump(LGB_result, f)

    X, y = get_data(last_date, 6, 'H')
    LGB_result = LGB_kfold(X, y, param, n_splits)
    with open(f'models\kat_6h\model_{last_date}.pkl', 'wb') as f:
        pickle.dump(LGB_result, f)